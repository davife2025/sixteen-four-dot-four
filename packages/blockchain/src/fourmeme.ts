...
// These are called by the agent when Kimi K2 decides to act
// ============================================================
...
// four.meme contract addresses (mainnet — for testnet, use testnet deploy)
// Reference: four-meme.gitbook.io/four.meme/brand/protocol-integration
const TOKEN_MANAGER_HELPER3 = '0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24'
const EIP8004_REGISTRY      = '0x0000000000000000000000000000000000008004' // placeholder
...
// ── Auth helpers (four.meme API login flow) ───────────────
...
// ── SKILL 1: Create meme token (API + chain) ─────────────
...
export interface CreateTokenParams {
  ...
  feePlan?: boolean  // anti-sniper mode
}
...
export async function createMemeToken(params: CreateTokenParams): Promise<CreateTokenResult> {
  ...
  // four.meme create token API call
  const createRes = await axios.post(
    `${FOURMEME_BASE_URL}/v1/private/token/create`,
    {
      ...
      feePlan: params.feePlan ?? true,  // anti-sniper on by default
      tokenTaxInfo: {
  ...
  const preSaleWei = params.preSaleBnb
    ? ethers.parseEther(params.preSaleBnb)
    : 0n
...
export async function getTokenInfo(tokenAddress: string): Promise<TokenInfo> {
  ...
  const result = await (helper['getTokenInfo'] as (addr: string) => Promise<TokenInfo>)(tokenAddress)
  return result
}
...
export async function buyToken(
  ...
  return {
    txHash: receipt.hash,
    tokenAmount: minAmount, // actual amount parsed from events in production
  }
}
...
export async function getRecentEvents(fromBlock: number, toBlock?: number): Promise<TokenEvent[]> {
  // Uses Bitquery GraphQL for real-time event data
  ...
  // Parse and return events (simplified — full parsing in production)
...
export async function getTaxInfo(tokenAddress: string): Promise<TaxInfo> {
  ...
  const [feeRate, rateFounder, rateHolder, rateBurn, rateLiquidity, minShare, founder] =
    await Promise.all([
      ...
      contract['rateFounder']() as Promise<bigint>,
      contract['rateHolder']() as Promise<bigint>,
      contract['rateBurn']() as Promise<bigint>,
      contract['rateLiquidity']() as Promise<bigint>,
      contract['minShare']() as Promise<bigint>,
      contract['founder']() as Promise<string>,
    ])
...
// ── SKILLS 10 & 11: EIP-8004 identity NFT ────────────────
...
export async function registerAgentIdentity(
  ...
  // EIP-8004 registry — register agent with on-chain identity NFT
  // This unlocks Insider Phase access on four.meme
  const registryAbi = [
...
// ── Token image upload (required before create) ───────────
